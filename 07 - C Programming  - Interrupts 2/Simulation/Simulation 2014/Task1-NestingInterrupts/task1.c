...
int main() {
    ...
    _T2IF = 0; //
    _NSTDIS = 0; // disable nesting of interrupts flag
    _T1IE = 1; //Switch on timer 1 interrupt enable
    _T2IE = 1;

    //GET READY
    TMR1 = 5; //Let's give timer1 a head start (PRESCALE*TMR1 instruction cycles)
    TMR2 = 0;
...
void __attribute__((interrupt, no_auto_psv)) _T2Interrupt() {
    //TASK 5 - WHAT IS THE LATENCY (In Instruction Cycles)?
    display->attribute.D2 = !display->attribute.D2; //Toggle LED
