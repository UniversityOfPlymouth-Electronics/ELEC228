...
extern unsigned int do_delay_badly(const unsigned int D);
...
int main()
{
	...
	//Configure PORTB to be all digital IO
	AD1PCFG = 0xFFFF;
	...
	unsigned int units;
	unsigned int tens;
	unsigned int uButtonHasBeenPressed;
	...
	//Assume no button has been pressed at startup
	uButtonHasBeenPressed = 0;
	...
	while (1)
	{
		...
		for (n=0; n<100; n++) {
			...
			//delay - this is where we spend most of our time, so this is where we shall poll our button (not good practise)
    		uButtonHasBeenPressed = do_delay_badly(500);
    		
    		//Was a button pressed during the delay routine?
    		if (uButtonHasBeenPressed == 1)
    			break;							//If yes, break out of the for-loop
